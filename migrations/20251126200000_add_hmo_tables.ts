...
export async function up(knex: Knex): Promise<void> {
  ...
  await knex.schema.raw("CREATE INDEX IF NOT EXISTS idx_hmo_providers_code ON hmo_providers(code);");
  await knex.schema.raw("CREATE INDEX IF NOT EXISTS idx_hmo_providers_active ON hmo_providers(is_active);");
  await knex.schema.raw("CREATE INDEX IF NOT EXISTS idx_nhis_service_codes_code ON nhis_service_codes(code);");
  await knex.schema.raw("CREATE INDEX IF NOT EXISTS idx_nhis_service_codes_category ON nhis_service_codes(category);");
  await knex.schema.raw("CREATE INDEX IF NOT EXISTS idx_hmo_tariffs_lookup ON hmo_tariffs(hmo_provider_id, service_code_id, effective_from);");
  await knex.schema.raw("CREATE INDEX IF NOT EXISTS idx_hmo_claims_patient ON hmo_claims(patient_id);");
  await knex.schema.raw("CREATE INDEX IF NOT EXISTS idx_hmo_claims_status ON hmo_claims(status);");
  await knex.schema.raw("CREATE INDEX IF NOT EXISTS idx_hmo_claims_date ON hmo_claims(claim_date);");
  await knex.schema.raw("CREATE INDEX IF NOT EXISTS idx_hmo_preauth_patient ON hmo_preauthorizations(patient_id);");
  await knex.schema.raw("CREATE INDEX IF NOT EXISTS idx_hmo_preauth_code ON hmo_preauthorizations(authorization_code);");
  await knex.schema.raw("CREATE INDEX IF NOT EXISTS idx_referrals_patient ON referrals(patient_id);");
  await knex.schema.raw("CREATE INDEX IF NOT EXISTS idx_referrals_code ON referrals(referral_code);");
  await knex.schema.raw("CREATE INDEX IF NOT EXISTS idx_referrals_status ON referrals(status);");
}
