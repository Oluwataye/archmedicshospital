/**
 * Nigerian States and Local Government Areas (LGAs) Module
 * Complete data structure with utility functions
 */

export const nigerianStatesData = {
    "Abia": {
        capital: "Umuahia",
        code: "AB",
        lgas: [
            "Aba North", "Aba South", "Arochukwu", "Bende", "Ikwuano",
            "Isiala Ngwa North", "Isiala Ngwa South", "Isuikwuato", "Obi Ngwa",
            "Ohafia", "Osisioma Ngwa", "Ugwunagbo", "Ukwa East", "Ukwa West",
            "Umuahia North", "Umuahia South", "Umu Nneochi"
        ]
    },
    "Adamawa": {
        capital: "Yola",
        code: "AD",
        lgas: [
            "Demsa", "Fufore", "Ganye", "Girei", "Gombi", "Guyuk", "Hong",
            "Jada", "Lamurde", "Madagali", "Maiha", "Mayo-Belwa", "Michika",
            "Mubi North", "Mubi South", "Numan", "Shelleng", "Song",
            "Toungo", "Yola North", "Yola South"
        ]
    },
    "Akwa Ibom": {
        capital: "Uyo",
        code: "AK",
        lgas: [
            "Abak", "Eastern Obolo", "Eket", "Esit Eket", "Essien Udim",
            "Etim Ekpo", "Etinan", "Ibeno", "Ibesikpo Asutan", "Ibiono Ibom",
            "Ika", "Ikono", "Ikot Abasi", "Ikot Ekpene", "Ini", "Itu",
            "Mbo", "Mkpat Enin", "Nsit Atai", "Nsit Ibom", "Nsit Ubium",
            "Obot Akara", "Okobo", "Onna", "Oron", "Oruk Anam",
            "Udung Uko", "Ukanafun", "Uruan", "Urue-Offong/Oruko", "Uyo"
        ]
    },
    "Anambra": {
        capital: "Awka",
        code: "AN",
        lgas: [
            "Aguata", "Anambra East", "Anambra West", "Anaocha", "Awka North",
            "Awka South", "Ayamelum", "Dunukofia", "Ekwusigo", "Idemili North",
            "Idemili South", "Ihiala", "Njikoka", "Nnewi North", "Nnewi South",
            "Ogbaru", "Onitsha North", "Onitsha South", "Orumba North",
            "Orumba South", "Oyi"
        ]
    },
    "Bauchi": {
        capital: "Bauchi",
        code: "BA",
        lgas: [
            "Alkaleri", "Bauchi", "Bogoro", "Damban", "Darazo", "Dass",
            "Gamawa", "Ganjuwa", "Giade", "Itas/Gadau", "Jama'are", "Katagum",
            "Kirfi", "Misau", "Ningi", "Shira", "Tafawa Balewa", "Toro",
            "Warji", "Zaki"
        ]
    },
    "Bayelsa": {
        capital: "Yenagoa",
        code: "BY",
        lgas: [
            "Brass", "Ekeremor", "Kolokuma/Opokuma", "Nembe",
            "Ogbia", "Sagbama", "Southern Ijaw", "Yenagoa"
        ]
    },
    "Benue": {
        capital: "Makurdi",
        code: "BE",
        lgas: [
            "Ado", "Agatu", "Apa", "Buruku", "Gboko", "Guma", "Gwer East",
            "Gwer West", "Katsina-Ala", "Konshisha", "Kwande", "Logo",
            "Makurdi", "Obi", "Ogbadibo", "Ohimini", "Oju", "Okpokwu",
            "Otukpo", "Tarka", "Ukum", "Ushongo", "Vandeikya"
        ]
    },
    "Borno": {
        capital: "Maiduguri",
        code: "BO",
        lgas: [
            "Abadam", "Askira/Uba", "Bama", "Bayo", "Biu", "Chibok", "Damboa",
            "Dikwa", "Gubio", "Guzamala", "Gwoza", "Hawul", "Jere", "Kaga",
            "Kala/Balge", "Konduga", "Kukawa", "Kwaya Kusar", "Mafa",
            "Magumeri", "Maiduguri", "Marte", "Mobbar", "Monguno", "Ngala",
            "Nganzai", "Shani"
        ]
    },
    "Cross River": {
        capital: "Calabar",
        code: "CR",
        lgas: [
            "Abi", "Akamkpa", "Akpabuyo", "Bakassi", "Bekwarra", "Biase",
            "Boki", "Calabar Municipal", "Calabar South", "Etung", "Ikom",
            "Obanliku", "Obubra", "Obudu", "Odukpani", "Ogoja", "Yakurr", "Yala"
        ]
    },
    "Delta": {
        capital: "Asaba",
        code: "DE",
        lgas: [
            "Aniocha North", "Aniocha South", "Bomadi", "Burutu", "Ethiope East",
            "Ethiope West", "Ika North East", "Ika South", "Isoko North",
            "Isoko South", "Ndokwa East", "Ndokwa West", "Okpe", "Oshimili North",
            "Oshimili South", "Patani", "Sapele", "Udu", "Ughelli North",
            "Ughelli South", "Ukwuani", "Uvwie", "Warri North", "Warri South",
            "Warri South West"
        ]
    },
    "Ebonyi": {
        capital: "Abakaliki",
        code: "EB",
        lgas: [
            "Abakaliki", "Afikpo North", "Afikpo South", "Ebonyi", "Ezza North",
            "Ezza South", "Ikwo", "Ishielu", "Ivo", "Izzi", "Ohaozara", "Ohaukwu", "Onicha"
        ]
    },
    "Edo": {
        capital: "Benin City",
        code: "ED",
        lgas: [
            "Akoko-Edo", "Egor", "Esan Central", "Esan North-East", "Esan South-East",
            "Esan West", "Etsako Central", "Etsako East", "Etsako West",
            "Igueben", "Ikpoba-Okha", "Oredo", "Orhionmwon", "Ovia North-East",
            "Ovia South-West", "Owan East", "Owan West", "Uhunmwonde"
        ]
    },
    "Ekiti": {
        capital: "Ado-Ekiti",
        code: "EK",
        lgas: [
            "Ado-Ekiti", "Efon", "Ekiti East", "Ekiti South-West", "Ekiti West",
            "Emure", "Gbonyin", "Ido-Osi", "Ijero", "Ikere", "Ikole", "Ilejemeje",
            "Irepodun/Ifelodun", "Ise/Orun", "Moba", "Oye"
        ]
    },
    "Enugu": {
        capital: "Enugu",
        code: "EN",
        lgas: [
            "Aninri", "Awgu", "Enugu East", "Enugu North", "Enugu South",
            "Ezeagu", "Igbo Etiti", "Igbo Eze North", "Igbo Eze South",
            "Isi Uzo", "Nkanu East", "Nkanu West", "Nsukka", "Oji River",
            "Udenu", "Udi", "Uzo-Uwani"
        ]
    },
    "FCT": {
        capital: "Abuja",
        code: "FC",
        lgas: [
            "Abaji", "Abuja Municipal", "Bwari", "Gwagwalada", "Kuje", "Kwali"
        ]
    },
    "Gombe": {
        capital: "Gombe",
        code: "GO",
        lgas: [
            "Akko", "Balanga", "Billiri", "Dukku", "Funakaye", "Gombe",
            "Kaltungo", "Kwami", "Nafada", "Shongom", "Yamaltu/Deba"
        ]
    },
    "Imo": {
        capital: "Owerri",
        code: "IM",
        lgas: [
            "Aboh Mbaise", "Ahiazu Mbaise", "Ehime Mbano", "Ezinihitte Mbaise",
            "Ideato North", "Ideato South", "Ihitte/Uboma", "Ikeduru", "Isiala Mbano",
            "Isu", "Mbaitoli", "Ngor Okpala", "Njaba", "Nkwerre", "Nwangele",
            "Obowo", "Oguta", "Ohaji/Egbema", "Okigwe", "Onuimo", "Orlu",
            "Orsu", "Oru East", "Oru West", "Owerri Municipal", "Owerri North",
            "Owerri West", "Unuimo"
        ]
    },
    "Jigawa": {
        capital: "Dutse",
        code: "JI",
        lgas: [
            "Auyo", "Babura", "Biriniwa", "Birnin Kudu", "Buji", "Dutse",
            "Gagarawa", "Garki", "Gumel", "Guri", "Gwaram", "Gwiwa",
            "Hadejia", "Jahun", "Kafin Hausa", "Kaugama", "Kazaure", "Kiri Kasama",
            "Kiyawa", "Maigatari", "Malam Madori", "Miga", "Ringim",
            "Roni", "Sule Tankarkar", "Taura", "Yankwashi"
        ]
    },
    "Kaduna": {
        capital: "Kaduna",
        code: "KD",
        lgas: [
            "Birnin Gwari", "Chikun", "Giwa", "Igabi", "Ikara", "Jaba",
            "Jema'a", "Kachia", "Kaduna North", "Kaduna South", "Kagarko",
            "Kajuru", "Kaura", "Kauru", "Kubau", "Kudan", "Lere",
            "Makarfi", "Sabon Gari", "Sanga", "Soba", "Zangon Kataf", "Zaria"
        ]
    },
    "Kano": {
        capital: "Kano",
        code: "KN",
        lgas: [
            "Ajingi", "Albasu", "Bagwai", "Bebeji", "Bichi", "Bunkure",
            "Dala", "Dambatta", "Dawakin Kudu", "Dawakin Tofa", "Doguwa",
            "Fagge", "Gabasawa", "Garko", "Garun Mallam", "Gaya", "Gezawa",
            "Gwale", "Gwarzo", "Kabo", "Kano Municipal", "Karaye", "Kibiya",
            "Kiru", "Kumbotso", "Kunchi", "Kura", "Madobi", "Makoda",
            "Minjibir", "Nasarawa", "Rano", "Rimin Gado", "Rogo", "Shanono",
            "Sumaila", "Takai", "Tarauni", "Tofa", "Tsanyawa", "Tudun Wada",
            "Ungogo", "Warawa", "Wudil"
        ]
    },
    "Katsina": {
        capital: "Katsina",
        code: "KT",
        lgas: [
            "Bakori", "Batagarawa", "Batsari", "Baure", "Bindawa", "Charanchi",
            "Dan Musa", "Dandume", "Danja", "Daura", "Dutsi", "Dutsin-Ma",
            "Faskari", "Funtua", "Ingawa", "Jibia", "Kafur", "Kaita",
            "Kankara", "Kankia", "Katsina", "Kurfi", "Kusada", "Mai'Adua",
            "Malumfashi", "Mani", "Mashi", "Matazu", "Musawa", "Rimi",
            "Sabuwa", "Safana", "Sandamu", "Zango"
        ]
    },
    "Kebbi": {
        capital: "Birnin Kebbi",
        code: "KE",
        lgas: [
            "Aleiro", "Arewa Dandi", "Argungu", "Augie", "Bagudo",
            "Birnin Kebbi", "Bunza", "Dandi", "Fakai", "Gwandu", "Jega",
            "Kalgo", "Koko/Besse", "Maiyama", "Ngaski", "Sakaba", "Shanga",
            "Suru", "Wasagu/Danko", "Yauri", "Zuru"
        ]
    },
    "Kogi": {
        capital: "Lokoja",
        code: "KO",
        lgas: [
            "Adavi", "Ajaokuta", "Ankpa", "Bassa", "Dekina", "Ibaji",
            "Idah", "Igalamela-Odolu", "Ijumu", "Kabba/Bunu", "Kogi", "Lokoja",
            "Mopa-Muro", "Ofu", "Ogori/Magongo", "Okehi", "Okene", "Olamaboro",
            "Omala", "Yagba East", "Yagba West"
        ]
    },
    "Kwara": {
        capital: "Ilorin",
        code: "KW",
        lgas: [
            "Asa", "Baruten", "Edu", "Ekiti", "Ifelodun", "Ilorin East",
            "Ilorin South", "Ilorin West", "Irepodun", "Isin", "Kaiama",
            "Moro", "Offa", "Oke Ero", "Oyun", "Pategi"
        ]
    },
    "Lagos": {
        capital: "Ikeja",
        code: "LA",
        lgas: [
            "Agege", "Ajeromi-Ifelodun", "Alimosho", "Amuwo-Odofin", "Apapa",
            "Badagry", "Epe", "Eti-Osa", "Ibeju-Lekki", "Ifako-Ijaiye",
            "Ikeja", "Ikorodu", "Kosofe", "Lagos Island", "Lagos Mainland",
            "Mushin", "Ojo", "Oshodi-Isolo", "Shomolu", "Surulere"
        ]
    },
    "Nasarawa": {
        capital: "Lafia",
        code: "NA",
        lgas: [
            "Akwanga", "Awe", "Doma", "Karu", "Keana", "Keffi", "Kokona",
            "Lafia", "Nasarawa", "Nasarawa Egon", "Obi", "Toto", "Wamba"
        ]
    },
    "Niger": {
        capital: "Minna",
        code: "NI",
        lgas: [
            "Agaie", "Agwara", "Bida", "Borgu", "Bosso", "Chanchaga", "Edati",
            "Gbako", "Gurara", "Katcha", "Kontagora", "Lapai", "Lavun",
            "Magama", "Mariga", "Mashegu", "Mokwa", "Munya", "Paikoro",
            "Rafi", "Rijau", "Shiroro", "Suleja", "Tafa", "Wushishi"
        ]
    },
    "Ogun": {
        capital: "Abeokuta",
        code: "OG",
        lgas: [
            "Abeokuta North", "Abeokuta South", "Ado-Odo/Ota", "Egbado North",
            "Egbado South", "Ewekoro", "Ifo", "Ijebu East", "Ijebu North",
            "Ijebu North East", "Ijebu Ode", "Ikenne", "Imeko Afon",
            "Ipokia", "Obafemi Owode", "Odeda", "Odogbolu", "Ogun Waterside",
            "Remo North", "Shagamu"
        ]
    },
    "Ondo": {
        capital: "Akure",
        code: "ON",
        lgas: [
            "Akoko North-East", "Akoko North-West", "Akoko South-East",
            "Akoko South-West", "Akure North", "Akure South", "Ese Odo",
            "Idanre", "Ifedore", "Ilaje", "Ile Oluji/Okeigbo", "Irele",
            "Odigbo", "Okitipupa", "Ondo East", "Ondo West", "Ose", "Owo"
        ]
    },
    "Osun": {
        capital: "Osogbo",
        code: "OS",
        lgas: [
            "Aiyedaade", "Aiyedire", "Atakumosa East", "Atakumosa West",
            "Boluwaduro", "Boripe", "Ede North", "Ede South", "Egbedore",
            "Ejigbo", "Ife Central", "Ife East", "Ife North", "Ife South",
            "Ifedayo", "Ifelodun", "Ila", "Ilesa East", "Ilesa West",
            "Irepodun", "Irewole", "Isokan", "Iwo", "Obokun", "Odo Otin",
            "Ola Oluwa", "Olorunda", "Oriade", "Orolu", "Osogbo"
        ]
    },
    "Oyo": {
        capital: "Ibadan",
        code: "OY",
        lgas: [
            "Afijio", "Akinyele", "Atiba", "Atisbo", "Egbeda", "Ibadan North",
            "Ibadan North-East", "Ibadan North-West", "Ibadan South-East",
            "Ibadan South-West", "Ibarapa Central", "Ibarapa East",
            "Ibarapa North", "Ido", "Irepo", "Iseyin", "Itesiwaju", "Iwajowa",
            "Kajola", "Lagelu", "Ogbomosho North", "Ogbomosho South",
            "Ogo Oluwa", "Olorunsogo", "Oluyole", "Ona Ara", "Orelope",
            "Ori Ire", "Oyo East", "Oyo West", "Saki East", "Saki West", "Surulere"
        ]
    },
    "Plateau": {
        capital: "Jos",
        code: "PL",
        lgas: [
            "Barkin Ladi", "Bassa", "Bokkos", "Jos East", "Jos North",
            "Jos South", "Kanam", "Kanke", "Langtang North", "Langtang South",
            "Mangu", "Mikang", "Pankshin", "Qua'an Pan", "Riyom", "Shendam", "Wase"
        ]
    },
    "Rivers": {
        capital: "Port Harcourt",
        code: "RI",
        lgas: [
            "Abua/Odual", "Ahoada East", "Ahoada West", "Akuku Toru", "Andoni",
            "Asari-Toru", "Bonny", "Degema", "Eleme", "Emohua", "Etche",
            "Gokana", "Ikwerre", "Khana", "Obio/Akpor", "Ogba/Egbema/Ndoni",
            "Ogu/Bolo", "Okrika", "Omuma", "Opobo/Nkoro", "Oyigbo",
            "Port Harcourt", "Tai"
        ]
    },
    "Sokoto": {
        capital: "Sokoto",
        code: "SO",
        lgas: [
            "Binji", "Bodinga", "Dange Shuni", "Gada", "Goronyo", "Gudu",
            "Gwadabawa", "Illela", "Isa", "Kebbe", "Kware", "Rabah",
            "Sabon Birni", "Shagari", "Silame", "Sokoto North", "Sokoto South",
            "Tambuwal", "Tangaza", "Tureta", "Wamako", "Wurno", "Yabo"
        ]
    },
    "Taraba": {
        capital: "Jalingo",
        code: "TA",
        lgas: [
            "Ardo Kola", "Bali", "Donga", "Gashaka", "Gassol", "Ibi",
            "Jalingo", "Karim Lamido", "Kurmi", "Lau", "Sardauna", "Takum",
            "Ussa", "Wukari", "Yorro", "Zing"
        ]
    },
    "Yobe": {
        capital: "Damaturu",
        code: "YO",
        lgas: [
            "Bade", "Bursari", "Damaturu", "Fika", "Fune", "Geidam",
            "Gujba", "Gulani", "Jakusko", "Karasuwa", "Machina", "Nangere",
            "Nguru", "Potiskum", "Tarmuwa", "Yunusari", "Yusufari"
        ]
    },
    "Zamfara": {
        capital: "Gusau",
        code: "ZA",
        lgas: [
            "Anka", "Bakura", "Birnin Magaji/Kiyaw", "Bukkuyum", "Bungudu",
            "Gummi", "Gusau", "Kaura Namoda", "Maradun", "Maru", "Shinkafi",
            "Talata Mafara", "Tsafe", "Zurmi"
        ]
    }
};

/**
 * Utility Functions
 */

// Get all states
export const getAllStates = () => {
    return Object.keys(nigerianStatesData).sort();
};

// Get LGAs for a specific state
export const getLGAsByState = (stateName: string) => {
    const state = nigerianStatesData[stateName as keyof typeof nigerianStatesData];
    return state ? state.lgas : [];
};

// Get state information
export const getStateInfo = (stateName: string) => {
    return nigerianStatesData[stateName as keyof typeof nigerianStatesData] || null;
};

// Search states by name
export const searchStates = (query: string) => {
    const lowercaseQuery = query.toLowerCase();
    return getAllStates().filter(state =>
        state.toLowerCase().includes(lowercaseQuery)
    );
};

// Search LGAs across all states
export const searchLGAs = (query: string) => {
    const lowercaseQuery = query.toLowerCase();
    const results: { state: string; lgas: string[] }[] = [];

    Object.entries(nigerianStatesData).forEach(([state, data]) => {
        const matchingLGAs = data.lgas.filter(lga =>
            lga.toLowerCase().includes(lowercaseQuery)
        );

        if (matchingLGAs.length > 0) {
            results.push({ state, lgas: matchingLGAs });
        }
    });

    return results;
};

// Find which state an LGA belongs to
export const findStateByLGA = (lgaName: string) => {
    const lowercaseLGA = lgaName.toLowerCase();

    for (const [state, data] of Object.entries(nigerianStatesData)) {
        const found = data.lgas.find(lga =>
            lga.toLowerCase() === lowercaseLGA
        );
        if (found) {
            return { state, lga: found };
        }
    }

    return null;
};

// Get total count of LGAs
export const getTotalLGACount = () => {
    return Object.values(nigerianStatesData).reduce(
        (total, state) => total + state.lgas.length, 0
    );
};

// Validate state and LGA combination
export const isValidStateLGA = (stateName: string, lgaName: string) => {
    const state = nigerianStatesData[stateName as keyof typeof nigerianStatesData];
    if (!state) return false;

    return state.lgas.some(lga =>
        lga.toLowerCase() === lgaName.toLowerCase()
    );
};

// Get states by geopolitical zone
export const getStatesByZone = (zone: string) => {
    const zones: Record<string, string[]> = {
        "North Central": ["Benue", "FCT", "Kogi", "Kwara", "Nasarawa", "Niger", "Plateau"],
        "North East": ["Adamawa", "Bauchi", "Borno", "Gombe", "Taraba", "Yobe"],
        "North West": ["Jigawa", "Kaduna", "Kano", "Katsina", "Kebbi", "Sokoto", "Zamfara"],
        "South East": ["Abia", "Anambra", "Ebonyi", "Enugu", "Imo"],
        "South South": ["Akwa Ibom", "Bayelsa", "Cross River", "Delta", "Edo", "Rivers"],
        "South West": ["Ekiti", "Lagos", "Ogun", "Ondo", "Osun", "Oyo"]
    };

    return zones[zone] || [];
};

// Format data for dropdown/select components
export const getStatesForDropdown = () => {
    return getAllStates().map(state => ({
        value: state,
        label: state,
        code: nigerianStatesData[state as keyof typeof nigerianStatesData].code
    }));
};

export const getLGAsForDropdown = (stateName: string) => {
    const lgas = getLGAsByState(stateName);
    return lgas.map(lga => ({
        value: lga,
        label: lga
    }));
};

// Legacy exports for backwards compatibility
export const NIGERIAN_STATES = getStatesForDropdown();
export const LGA_BY_STATE: Record<string, string[]> = Object.fromEntries(
    Object.entries(nigerianStatesData).map(([state, data]) => [state, data.lgas])
);

export const RELIGIONS = [
    { value: "Christianity", label: "Christianity" },
    { value: "Islam", label: "Islam" },
    { value: "Traditional", label: "Traditional" },
    { value: "Other", label: "Other" },
    { value: "Prefer not to say", label: "Prefer not to say" },
] as const;

export const EMPLOYMENT_STATUS = [
    { value: "Employed", label: "Employed" },
    { value: "Self-Employed", label: "Self-Employed" },
    { value: "Unemployed", label: "Unemployed" },
    { value: "Student", label: "Student" },
    { value: "Retired", label: "Retired" },
] as const;

export const NOK_RELATIONSHIPS = [
    { value: "Spouse", label: "Spouse" },
    { value: "Parent", label: "Parent" },
    { value: "Sibling", label: "Sibling" },
    { value: "Child", label: "Child" },
    { value: "Friend", label: "Friend" },
    { value: "Other", label: "Other" },
] as const;

export const NIGERIAN_TRIBES = [
    "Yoruba", "Igbo", "Hausa", "Fulani", "Ijaw", "Kanuri", "Ibibio",
    "Tiv", "Edo", "Nupe", "Urhobo", "Igala", "Idoma", "Efik",
    "Annang", "Itsekiri", "Jukun", "Gwari", "Ebira", "Isoko"
];

// Export default object with all functions
export default {
    data: nigerianStatesData,
    getAllStates,
    getLGAsByState,
    getStateInfo,
    searchStates,
    searchLGAs,
    findStateByLGA,
    getTotalLGACount,
    isValidStateLGA,
    getStatesByZone,
    getStatesForDropdown,
    getLGAsForDropdown
};
