...
var DoctorDashboardPage = function () {
    ...
    var getButtonClass = function (priority) {
        return priority === 'urgent'
            ? 'bg-red-500 text-white px-4 py-1 rounded'
            : 'bg-blue-500 text-white px-4 py-1 rounded';
    };
    var getRowClass = function (index, priority) {
        if (index === 0)
            return 'border-b border-t border-2 border-blue-500';
        if (priority === 'urgent')
      ...
      <card_1.Card className="mb-6">
        <div className="p-6 flex flex-col md:flex-row md:justify-between md:items-center space-y-4 md:space-y-0">
          ...
          <div className="relative">
            ...
            <input type="text" value={searchQuery} onChange={function (e) { return setSearchQuery(e.target.value); }} className="block w-full md:w-64 pl-10 pr-3 py-2 rounded-full bg-gray-100 border border-gray-200" placeholder="Search patients..."/>
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody>
              {patientQueue.map(function (patient, index) { return (<tr key={patient.id} className={getRowClass(index, patient.priority)}>
          ...
          <pagination_1.Pagination>
            <pagination_1.PaginationContent>
              <pagination_1.PaginationItem>
                <pagination_1.PaginationLink isActive className="w-10 h-10 flex items-center justify-center rounded bg-blue-500 text-white">
                  1
                </pagination_1.PaginationLink>
              </pagination_1.PaginationItem>
              <pagination_1.PaginationItem>
                <pagination_1.PaginationLink className="w-10 h-10 flex items-center justify-center rounded border border-gray-200 text-gray-500">
                  2
                </pagination_1.PaginationLink>
              </pagination_1.PaginationItem>
              <pagination_1.PaginationItem>
              ...
              <pagination_1.PaginationItem>
                <pagination_1.PaginationNext className="w-10 h-10 flex items-center justify-center rounded border border-gray-200 text-gray-500">
                  <lucide_react_1.ChevronRight className="h-4 w-4"/>
                </pagination_1.PaginationNext>
              </pagination_1.PaginationItem>
            </pagination_1.PaginationContent>
          </pagination_1.Pagination>
        </div>
      ...
      <card_1.Card>
        <card_1.CardContent className="p-6">
          ...
          <div className="ml-4 relative border-l-2 border-gray-200 pl-6">
            {activities.map(function (activity) { return (<div key={activity.id} className="mb-6 last:mb-0 relative">
                <div className={"absolute -left-10 top-0 w-4 h-4 rounded-full bg-".concat(activity.color, "-500")}></div>
