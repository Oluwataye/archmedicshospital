...
var EquipmentPage = function () {
    ...
    var uniqueTypes = Array.from(new Set(allEquipment.map(function (eq) { return eq.equipmentType; })));
    // Filter equipment based on search query and filters
    var filteredEquipment = allEquipment.filter(function (equipment) {
        ...
        var typeMatch = typeFilter === 'all' ||
            equipment.equipmentType === typeFilter;
        return searchMatch && statusMatch && typeMatch;
    });
    // Handle schedule maintenance button
    var handleScheduleMaintenance = function (equipmentId) {
        sonner_1.toast.info("Scheduling maintenance for equipment ".concat(equipmentId));
    };
    // Handle view logs button
    var handleViewLogs = function (equipmentId) {
        sonner_1.toast.info("Viewing logs for equipment ".concat(equipmentId));
    };
    // Handle view details button
    var handleViewDetails = function (equipmentId) {
        sonner_1.toast.info("Viewing details for equipment ".concat(equipmentId));
    };
    // Handle adding new equipment
    var handleAddEquipment = function () {
        sonner_1.toast.info("Adding new equipment");
    };
    return (<div>
      {/* Breadcrumbs */}
      ...
      {allEquipment.some(function (eq) { return eq.status === 'Out of Service'; }) && (<div className="bg-red-50 border-l-4 border-red-500 p-4 mb-6">
          <div className="flex">
        ...
        <div className="flex flex-col md:flex-row gap-4">
          <div className="flex-grow">
            <div className="relative">
              ...
              <input_1.Input placeholder="Search by name, ID or model..." className="pl-9" value={searchQuery} onChange={function (e) { return setSearchQuery(e.target.value); }}/>
            </div>
          </div>
            ...
            <select_1.Select value={statusFilter} onValueChange={setStatusFilter}>
              ...
              <select_1.SelectContent>
                <select_1.SelectGroup>
                  ...
                  <select_1.SelectItem value="operational">Operational</select_1.SelectItem>
                  <select_1.SelectItem value="needs maintenance">Needs Maintenance</select_1.SelectItem>
                  <select_1.SelectItem value="out of service">Out of Service</select_1.SelectItem>
                </select_1.SelectGroup>
              </select_1.SelectContent>
            </select_1.Select>
          </div>
          ...
          <div className="md:w-1/5">
            <select_1.Select value={typeFilter} onValueChange={setTypeFilter}>
              <select_1.SelectTrigger>
                <select_1.SelectValue placeholder="Equipment Type"/>
              </select_1.SelectTrigger>
              <select_1.SelectContent>
                <select_1.SelectGroup>
                  <select_1.SelectLabel>Filter by Type</select_1.SelectLabel>
                  <select_1.SelectItem value="all">All Types</select_1.SelectItem>
                  {uniqueTypes.map(function (type, index) { return (<select_1.SelectItem key={index} value={type}>{type}</select_1.SelectItem>); })}
                </select_1.SelectGroup>
              </select_1.SelectContent>
            </select_1.Select>
          </div>
          
          <button_1.Button onClick={handleAddEquipment} className="md:w-auto" style={{ backgroundColor: '#3B82F6' }}>
            Add Equipment
          </button_1.Button>
        </div>
      </card_1.Card>
      ...
      <card_1.Card className="border border-gray-200 mb-6">
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredEquipment.map(function (equipment) { return (<tr key={equipment.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    <span className={"px-2 inline-flex text-xs leading-5 font-semibold rounded-full ".concat(equipment.statusColor)}>
                      {equipment.status}
                    </span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    <button_1.Button variant="ghost" size="sm" className="text-blue-600 hover:text-blue-900 mr-2 p-0" onClick={function () { return handleScheduleMaintenance(equipment.id); }}>
                      <lucide_react_1.Calendar className="h-4 w-4 mr-1"/>
                      Schedule
                    </button_1.Button>
                    <button_1.Button variant="ghost" size="sm" className="text-green-600 hover:text-green-900 mr-2 p-0" onClick={function () { return handleViewLogs(equipment.id); }}>
                      Logs
                    </button_1.Button>
                    <button_1.Button variant="ghost" size="sm" className="text-gray-600 hover:text-gray-900 p-0" onClick={function () { return handleViewDetails(equipment.id); }}>
                      Details
                    </button_1.Button>
                  </td>
                </tr>); })}
        ...
        {filteredEquipment.length === 0 && (<div className="py-8 text-center text-gray-500">
            <p>No equipment found matching your criteria.</p>
          </div>)}
        
        {/* Pagination */}
        {filteredEquipment.length > 0 && (<div className="flex items-center justify-between p-4">
      ...
      <card_1.Card className="border border-gray-200 mb-6">
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {upcomingMaintenance.map(function (maintenance) { return (<tr key={maintenance.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    <button_1.Button variant="ghost" size="sm" className="text-blue-600 hover:text-blue-900 mr-2 p-0">
                      <lucide_react_1.Wrench className="h-4 w-4 mr-1"/>
                      Record
                    </button_1.Button>
                    <button_1.Button variant="ghost" size="sm" className="text-gray-600 hover:text-gray-900 p-0">
                      Details
                    </button_1.Button>
                  </td>
                </tr>); })}
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <card_1.Card className="p-4 border border-green-200 bg-green-50">
          ...
          <p className="text-2xl font-bold text-green-700">
            {allEquipment.filter(function (eq) { return eq.status === 'Operational'; }).length}
          </p>
        </card_1.Card>
        <card_1.Card className="p-4 border border-yellow-200 bg-yellow-50">
          ...
          <p className="text-2xl font-bold text-yellow-700">
            {allEquipment.filter(function (eq) { return eq.status === 'Needs Maintenance'; }).length}
          </p>
        </card_1.Card>
        <card_1.Card className="p-4 border border-red-200 bg-red-50">
          ...
          <p className="text-2xl font-bold text-red-700">
            {allEquipment.filter(function (eq) { return eq.status === 'Out of Service'; }).length}
          </p>
        </card_1.Card>
