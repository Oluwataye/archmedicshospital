...
try {
    console.log('Fixing revolving_funds foreign key constraint...\n');

    // Get current data
    const funds = db.prepare('SELECT * FROM revolving_funds').all();
    const transactions = db.prepare('SELECT * FROM revolving_fund_transactions').all();

    console.log(`Found ${funds.length} funds and ${transactions.length} transactions`);

    // Drop and recreate tables without the units FK
    db.exec('DROP TABLE IF EXISTS revolving_fund_transactions');
    db.exec('DROP TABLE IF EXISTS revolving_funds');

    // Recreate revolving_funds table with wards reference
    db.exec(`
